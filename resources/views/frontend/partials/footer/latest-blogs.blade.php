                ...
                @foreach ($latestBlogInfos as $latestBlogInfo)
                    <div class="item mt-20 d-flex align-items-center">
                        <div class="blog-img {{ $currentLanguageInfo->direction == 0 ? 'mr-4' : 'ml-4' }}">
                            <img data-src="{{ asset('/img/blogs/' . $latestBlogInfo->image) }}" class="lazy"
                                 alt="image">
                        </div>
                        ...
                        <div class="blog-info">
                            <h6 class="blog-title">
                                <a
                                        href="{{ route('blog_details', ['slug' => $latestBlogInfo->slug]) }}">{{ strlen($latestBlogInfo->title) > 30 ? mb_substr($latestBlogInfo->title, 0, 30, 'UTF-8') . '...' : $latestBlogInfo->title }}</a>
                            </h6>
