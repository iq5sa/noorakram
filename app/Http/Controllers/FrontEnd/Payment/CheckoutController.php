  ...
  public function submit(Request $request, $order_id)
  {
    ...
    switch ($request->get('payment_method')) {
      case "cash":
        ...
        return (new CashController())->pay($request->get('cash_code'), $order_id);
      case "zain_cash":

        return (new ZainCashController())->pay($discount ?? $course_price, $order_id, $orderType);
      case "credit":

        $credit = new TabadulPayment();
        $amount = (int)($discount ?? $course_price);
