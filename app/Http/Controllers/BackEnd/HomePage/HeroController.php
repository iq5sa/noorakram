...
class HeroController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        // insert data into db
        if (empty($heroInfo)) {
            $backgroundImageName = UploadFile::store('./img/hero-section/', $request->file('background_image'));

            $imageName = NULL;

            if ($request->hasFile('image')) {
                $imageName = UploadFile::store('./img/hero-section/', $request->file('image'));
            }
            ...
            if ($request->hasFile('background_image')) {
                $backgroundImageName = UploadFile::update('./img/hero-section/', $request->file('background_image'), $heroInfo->background_image);
            }

            if ($request->hasFile('image')) {
                $imageName = UploadFile::update('./img/hero-section/', $request->file('image'), $heroInfo->image);
            }
